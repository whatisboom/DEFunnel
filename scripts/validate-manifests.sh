...
#
# Libs/* references are skipped — the BigWigsMods packager fetches those
# externals at release time; they are not present in the repo.
set -euo pipefail
...
for toc in *.toc; do
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    [[ "$trimmed" == \#* ]] && continue
    path="$(norm "$trimmed")"
    if is_lib_ref "$path"; then continue; fi
    printf '%s\n' "$path" >> "$refs_file"
    if [[ ! -f "$path" ]]; then
...
while IFS= read -r xml; do
  ...
  while IFS= read -r ref; do
    [[ -z "$ref" ]] && continue
    ref_norm="$(norm "$ref")"
    if is_lib_ref "$ref_norm"; then continue; fi
    if [[ "$dir" == "." ]]; then
      resolved="$ref_norm"
    else
      resolved="$dir/$ref_norm"
    fi
    if is_lib_ref "$(norm "$resolved")"; then continue; fi
    printf '%s\n' "$resolved" >> "$refs_file"
    if [[ ! -f "$resolved" ]]; then
